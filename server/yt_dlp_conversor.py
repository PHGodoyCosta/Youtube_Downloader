...
class YoutubeDownloader:
    ...
    def download_sound(self, url):
        try:
            ydl_opts = {
                ...
                'outtmpl': '%(title)s.mp3',  # Nome do arquivo de saída
            }
    ...
    def download_mp4(self, url):
        try:
            ydl_opts = {
                'format': 'bestvideo[height<=720]+bestaudio[ext=m4a]/mp4',  # Forçar H.264 e MP4 -> bv*[vcodec^=avc1]+ba/b[ext=mp4]/worst
                'merge_output_format': 'mp4',  # Combinar vídeo e áudio em MP4
                'outtmpl': '%(title)s.%(ext)s',  # Nome do arquivo de saída
                'postprocessors': [{
                    'key': 'FFmpegVideoConvertor',
                    'preferedformat': 'mp4',  # Garante que o formato final seja MP4
                }]
            }
