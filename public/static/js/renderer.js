document.addEventListener("DOMContentLoaded", (e) => {
    ...
    const playlistProgressTitle = document.getElementById(
        "playlist-progress-title",
    );
    const playlistLoaderProgress = document.getElementById(
        "playlist-loader-progress",
    );
    ...
    async function getVideoInformations(videoId) {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(
                    "Erro ao fazer a requisição: " + response.statusText,
                );
            }
    ...
    function dispair_erro(code = 0, message = "") {
        ...
        switch (code) {
            case 0:
                textError =
                    "É para colocar um link Maria, não bater a cabeça no teclado e dar enter! 🙄";
                break;
            case 1:
                textError =
                    "Parece que esse erro é mais avançado, você vai ter que chamar seu Godoyzinho querido para resolver 😁";
                poster = "error-1";
                break;
            case 2:
        ...
        if (converterOptions.typeUnity == "musica") {
            ...
            if (checkYoutubeURL(text)) {
                ...
                if (videoId) {
                    ...
                    let queueItem = await appendQueue(
                        informations.title,
                    ...
                    createMusicLoadingElement(
                        informations.poster,
                        informations.title,
                        (duration = informations.duration),
                        (type = converterOptions.type),
                    );

                    let c = await converting(
            ...
            for (let i = 0; i < informations.playlist_count; i++) {
                ...
                if (c.status == "ok") {
                    centerArea.replaceChildren();
                    await changeQueueStatus(queueItem.hash, "concluida");
                    atualizandoPlaylistProgress(
                        i + 1,
                        informations.playlist_count,
                    );
                } else {
                    await changeQueueStatus(queueItem.hash, "error");
                    dispair_erro(3, (message = `${c.error}<br>Link: ${text}`));
    ...
    function createMusicLoadingElement(
        poster,
        title,
        duration = "00:00",
        type = "mp3",
        ...
        loadingTextDiv.innerHTML =
            'Carregando<span class="dot">.</span><span class="dot">.</span><span class="dot">.</span>';
    ...
    function createLoadingPlaylistElement(message = "Carregando Playlist") {
        ...
        container.className =
            "col d-flex flex-column justify-content-center align-items-center mt-5";
    ...
    async function converting(
        url,
        type = converterOptions.type,
        typeUnity = converterOptions.typeUnity,
        downloadPath = converterOptions.downloadPath,
    ) {
        try {
    ...
    async function getPlaylistInformations(url) {
        try {
            const response = await fetch(
                `${apiServer}/get_playlist_informations`,
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
